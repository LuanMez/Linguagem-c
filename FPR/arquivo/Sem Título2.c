...
int criando(char nomeArq[])
{
	...
	FILE *arquivo, *arquivo2, *arqnovo;
	int continuar=TRUE, cont=0;//opção verdadeira do continuar
	float maior, numero, numero2, ultimo = MINIMO; // maior numero float, menor seria FLT_MIN
	...
	rewind(arquivo);//voltar com o cursor para a primeira posição do arquivo
	rewind(arquivo2);//melhor do que fazer fclose e fopen denovo
	
	while((fscanf(arquivo,"%f",&numero) != EOF) && cont!=0)
	{
		while((fscanf(arquivo2,"%f",&numero2) != EOF))
		{
			if(numero==numero2)
			{
				fprintf(arqnovo,"%.1f",numero);
				cont--;
			}
		}
	}
