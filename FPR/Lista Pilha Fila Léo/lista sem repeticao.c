...
int remover (TLista *L, int numero)
{
	...
	if (*L)
	{
		//verificando se 'numero' é o primeiro elemento da lista. 
		if ((*L)->valor == numero)
		{
			atual = *L;
	...
	//remoção não foi realizada, pois o número não foi encontrado
	return FALSE;
}
...
int alterar (TLista L, int num1, int num2)
{
	...
	if (pos1)
	{
		...
		//se 'num2' não existir, a alteração pode ser realizada
		if (!pos2)
		{
			pos1->valor = num2;
